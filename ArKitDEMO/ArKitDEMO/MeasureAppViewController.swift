//
//  MeasureAppViewController.swift
//  ArKitDEMO
//
//  Created by Jay Buddhdev on 24/08/22.
//
...
class MeasureAppViewController: UIViewController,ARSCNViewDelegate {
    ...
    @objc func tapped(reconginzer:UITapGestureRecognizer){
        let sceneView = reconginzer.view as! ARSCNView
        ...
        if !hitTestResults.isEmpty {
            ...
            if self.spheres.count == 2 {
                ...
                let centrePoint = SCNVector3((firstPoint.position.x+secondPoint.position.x)/2,(firstPoint.position.y+secondPoint.position.y)/2,(firstPoint.position.z+secondPoint.position.z)/2)
                display(distance: result, position:centrePoint)
            }
        }
    }
